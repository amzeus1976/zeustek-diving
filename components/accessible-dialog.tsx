...
export function AccessibleDialog({label,className,close,editable=false,dirty=false,children}:{label:string;className:string;close:()=>void;editable?:boolean;dirty?:boolean;children:ReactNode}){
  ...
  return <dialog tabIndex={-1} ref={ref} className={className} aria-label={label} data-editable={editable||undefined}
    onCancel={event=>{event.preventDefault();if(dialogAllowsImplicitDismiss(editable))close();}}
