...
function TripDetail({ item, plans, sites, people, equipment, equipmentSets, close, edit, remove, togglePacked, addDocuments, removeDocument }: {
  ...
  return <AccessibleDialog className={`focus-modal ${styles.detail}`} label={`${item.name} trip details`} close={close}>
    ...
    <section><h3>Gas &amp; fill logistics</h3>{item.gasLogistics.length?<div className={styles.stack}>{item.gasLogistics.map((gas)=><article key={gas.id}><b>{gas.label || gas.gas || 'Gas requirement'}</b><span>{[gas.gas,gas.plannedFillBar!=null?`${gas.plannedFillBar} bar`:'',gas.fillProvider,gas.booked?'Booked':''].filter(Boolean).join(' · ')}</span>{gas.notes&&<small>{gas.notes}</small>}</article>)}</div>:<p className="focus-copy">No gas logistics recorded.</p>}</section>

    <section><h3>Bookings &amp; accommodation</h3>{item.accommodation&&<p><b>Accommodation:</b> {item.accommodation}</p>}{item.bookings.length?<div className={styles.stack}>{item.bookings.map((booking)=><article key={booking.id}><b>{booking.provider || booking.kind}</b><span>{[booking.reference,booking.amount!=null?`${booking.currency||'GBP'} ${booking.amount.toFixed(2)}`:'',booking.paid?'Paid':'',booking.dueOn?`Due ${booking.dueOn}`:''].filter(Boolean).join(' · ')}</span>{booking.notes&&<small>{booking.notes}</small>}</article>)}</div>:<p className="focus-copy">No bookings recorded.</p>}</section>

    <section><h3>Documents</h3><p className="focus-copy">PDFs and evidence files remain attached to this Trip; metadata stays available offline when previously loaded.</p><MediaGallery ownerKind="dive-trip" ownerId={item.entityId} acceptFiles retainOfflineMetadata accessibleViewer onUploaded={addDocuments} onRemoved={removeDocument}/></section>

    <section className={styles.notes}><h3>Emergency, insurance &amp; notes</h3><div><small>Emergency</small><p>{item.emergencyNotes || 'Not recorded'}</p></div><div><small>Insurance</small><p>{item.insuranceNotes || 'Not recorded'}</p></div><div><small>Medical / travel documents</small><p>{item.medicalNotes || 'Not recorded'}</p></div><div><small>Trip notes</small><p>{item.notes || 'Not recorded'}</p></div></section>

    <footer><button className="focus-secondary danger" onClick={remove}><Trash2 size={15}/> Delete</button><span/><button className="focus-secondary" onClick={close}>Close</button><button className="focus-primary" onClick={edit}><Pencil size={15}/> Edit trip</button></footer>
  </AccessibleDialog>;
}
...
function EditorChoices({ title, icon, empty, children }: { title:string; icon:React.ReactNode; empty:string; children:React.ReactNode }) {
  ...
  return <Card className={styles.choiceSection}><div className="focus-card-head"><div>{icon}<h3>{title}</h3></div></div>{array.length > 10 && <label>Search {title.toLowerCase()}<input type="search" value={search} onChange={event => setSearch(event.target.value)}/></label>}<div className={styles.choiceGrid}>{visible.length ? visible : <p className="focus-copy">{term ? 'No matching records.' : empty}</p>}</div></Card>;
}
function RepeatSection({ title, addLabel, add, children }: { title:string; addLabel:string; add:()=>void; children:React.ReactNode }) {
  return <Card className={styles.repeatSection}><div className="focus-card-head"><h3>{title}</h3><button className="focus-secondary" onClick={add}><Plus size={14}/>{addLabel}</button></div><div className={styles.stack}>{children}</div></Card>;
}
