...
function ActivityForm({item,initialType,linked,programmes,close,saved}:{item:ConservationActivity|null;initialType:ActivityType;linked:Linked;programmes:ProgrammeReference[];close:()=>void;saved:()=>Promise<void>}) {
  ...
  const [siteSearch,setSiteSearch]=useState(''); const [diveSearch,setDiveSearch]=useState('');
  ...
  const filteredSites=linked.sites.filter(s=>s.entityId===draft.siteId||s.name.toLocaleLowerCase().includes(siteSearch.toLocaleLowerCase()));
  const filteredDives=linked.dives.filter(d=>d.entityId===draft.diveId||[d.site,d.date].join(' ').toLocaleLowerCase().includes(diveSearch.toLocaleLowerCase()));
  return <div className="focus-modal-bg"><AccessibleDialog label={item?'Edit conservation activity':'Log conservation activity'} className="focus-modal conservation-form" close={()=>{if(!busy)close();}}>
    <form onSubmit={e=>void submit(e)}><header><h2>{item?'Edit conservation activity':'Log conservation activity'}</h2><button type="button" className="focus-icon" disabled={busy} aria-label="Close activity editor" onClick={close}><X/></button></header>
      <fieldset disabled={busy} className="record-fields"><label>Activity type<select value={draft.activityType} onChange={e=>field('activityType',e.target.value as ActivityType)}>{ACTIVITY_TYPES.map(([key,label])=><option key={key} value={key}>{label}</option>)}</select></label>
      <label>Date and time<input type="datetime-local" required value={localDate} onChange={e=>setLocalDate(e.target.value)}/></label>
      <label>Find an existing site<input type="search" value={siteSearch} onChange={e=>setSiteSearch(e.target.value)}/><select aria-label="Linked site" value={draft.siteId??''} onChange={e=>field('siteId',e.target.value||null)}><option value="">No linked site</option>{draft.siteId&&!linked.sites.some(s=>s.entityId===draft.siteId)&&<option value={draft.siteId}>Unavailable reference retained</option>}{filteredSites.map(s=><option key={s.entityId} value={s.entityId}>{s.name}</option>)}</select></label>
      <label>Find an existing Dive<input type="search" value={diveSearch} onChange={e=>setDiveSearch(e.target.value)}/><select aria-label="Linked Dive" value={draft.diveId??''} onChange={e=>field('diveId',e.target.value||null)}><option value="">No linked Dive</option>{draft.diveId&&!linked.dives.some(d=>d.entityId===draft.diveId)&&<option value={draft.diveId}>Unavailable reference retained</option>}{filteredDives.map(d=><option key={d.entityId} value={d.entityId}>{d.date} · {d.site}</option>)}</select></label>
      <label className="record-wide">{draft.activityType==='learning'?'Course / learning subject':'Species / subject'}<input value={draft.speciesOrSubject??''} onChange={e=>field('speciesOrSubject',e.target.value)}/></label>
      <label>Observation tags (comma separated)<input value={tagsText} onChange={e=>setTagsText(e.target.value)}/></label>
      <label>Verification state<select value={draft.verificationState??'recorded'} onChange={e=>field('verificationState',e.target.value as VerificationState)}><option value="recorded">Recorded, not independently verified</option><option value="verified">Verified evidence recorded</option><option value="needs-review">Needs review</option></select></label>
