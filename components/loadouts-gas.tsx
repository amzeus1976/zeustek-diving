...
function CylinderEditor({ item, close, saved }: { item: Stored<CylinderEquipmentRecord> | null; close: () => void; saved: () => Promise<void> | void }) {
  ...
  const existingSchedule = deriveCylinderInspectionDisplay(item ?? {});
...
function CylinderDetail({ item, fills, analyses, people, close, edit, remove, saved }: { item: Stored<CylinderEquipmentRecord>; fills: Array<Stored<CylinderFillRecord>>; analyses: Array<Stored<GasAnalysisRecord>>; people: Array<Stored<PersonRecord>>; close: () => void; edit: () => void; remove: () => void; saved: () => Promise<void> | void }) {
  ...
  const [savedFill, setSavedFill] = useState(fill);
  const [savedUsage, setSavedUsage] = useState(usage);
  const [savedAnalysis, setSavedAnalysis] = useState(analysis);
  const dirty = hasUnsavedCylinderDetailDraft({ profile, fill, usage, analysis }, { profile: savedProfile, fill: savedFill, usage: savedUsage, analysis: savedAnalysis });
  const [busy, setBusy] = useState(false); const [error, setError] = useState('');
  async function saveProfile() { setBusy(true); setError(''); try { await saveCylinderProfile({ ...item, entityId: item.entityId, waterVolumeLiters: profile.waterVolumeLiters ? Number(profile.waterVolumeLiters) : null, workingPressureBar: profile.workingPressureBar ? Number(profile.workingPressureBar) : null, cylinderMaterial: profile.cylinderMaterial, valveType: profile.valveType, oxygenClean: profile.oxygenClean, oxygenCleanUntil: profile.oxygenCleanUntil || null, lastTestType: profile.lastTestType as 'hydro' | 'visual', lastTestAt: profile.lastTestAt || null, hydroTestAt: profile.lastTestType === 'hydro' ? profile.lastTestAt || null : item.hydroTestAt ?? null, visualTestAt: profile.lastTestAt || null, tareKg: profile.tareKg ? Number(profile.tareKg) : null, owner: profile.owner, cylinderStatus: profile.cylinderStatus as NonNullable<CylinderEquipmentRecord['cylinderStatus']> }); await saved(); setSavedProfile(profile); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Cylinder profile could not be saved.'); } finally { setBusy(false); } }
  async function addFill(event: React.SyntheticEvent<HTMLFormElement>) { event.preventDefault(); setBusy(true); setError(''); try { const result = await saveCylinderFill({ cylinderEquipmentId: item.entityId, filledAt: new Date(fill.filledAt).toISOString(), pressureBar: fill.pressureBar ? Number(fill.pressureBar) : null, oxygenFraction: asFraction(fill.oxygenPercent), heliumFraction: asFraction(fill.heliumPercent), provider: fill.provider, notes: fill.notes, source: 'recorded' }); await saved(); const nextFill = { filledAt: localDateTime(), pressureBar: '', oxygenPercent: '', heliumPercent: '', provider: '', notes: '' }; setFill(nextFill); setSavedFill(nextFill); if (JSON.stringify(analysis) === JSON.stringify(savedAnalysis)) { const nextAnalysis = { ...analysis, fillId: result.id, analysedAt: localDateTime() }; setAnalysis(nextAnalysis); setSavedAnalysis(nextAnalysis); } } catch (reason) { setError(reason instanceof Error ? reason.message : 'Fill could not be saved.'); } finally { setBusy(false); } }
  async function addUsage(event: React.SyntheticEvent<HTMLFormElement>) { event.preventDefault(); setBusy(true); setError(''); try { await recordCylinderGasUsage({ cylinderEquipmentId: item.entityId, recordedAt: new Date(usage.recordedAt).toISOString(), pressureUsedBar: usage.pressureUsedBar ? Number(usage.pressureUsedBar) : null, remainingPressureBar: usage.remainingPressureBar ? Number(usage.remainingPressureBar) : null, notes: usage.notes }); await saved(); const nextUsage = { recordedAt: localDateTime(), pressureUsedBar: '', remainingPressureBar: '', notes: '' }; setUsage(nextUsage); setSavedUsage(nextUsage); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Gas use could not be saved.'); } finally { setBusy(false); } }
  async function addAnalysis(event: React.SyntheticEvent<HTMLFormElement>) { event.preventDefault(); setBusy(true); setError(''); try { await saveGasAnalysis({ cylinderEquipmentId: item.entityId, fillId: analysis.fillId || null, analysedAt: new Date(analysis.analysedAt).toISOString(), oxygenFraction: asFraction(analysis.oxygenPercent), heliumFraction: asFraction(analysis.heliumPercent), analysedByPersonId: analysis.analysedByPersonId || null, attachmentIds: [], notes: analysis.notes }); await saved(); const nextAnalysis = { fillId: analysis.fillId, analysedAt: localDateTime(), oxygenPercent: '', heliumPercent: '', analysedByPersonId: '', notes: '' }; setAnalysis(nextAnalysis); setSavedAnalysis(nextAnalysis); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Analysis could not be saved.'); } finally { setBusy(false); } }
  ...
  return <div className="focus-modal-bg"><AccessibleDialog editable dirty={dirty} trackInteractions={false} label={`${item.name} cylinder details`} className={`focus-modal ${styles.cylinderDetail}`} close={() => { if (!busy) close(); }}><header><div><span className="focus-eyebrow">CYLINDER / GAS</span><h2>{item.name}</h2><p>{[item.manufacturer, item.model, item.serialNumber && `S/N ${item.serialNumber}`].filter(Boolean).join(' · ')}</p><p>Save each section separately before closing.</p></div><button className="focus-icon" data-dialog-close aria-label="Close cylinder details" onClick={close}><X/></button></header>
