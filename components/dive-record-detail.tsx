...
export function DiveRecordDetail({ dive, title, eyebrow, rows, close, edit, remove, initialView = 'overview' }: {
  ...
  }, [dive, setEvidence, setSkills, setPeople, setEquipmentSets, setPlan, setContextError]);
  useEffect(() => {
    let active = true;
    const skillsChanged = () => { if (active) void refreshContext(); };
    queueMicrotask(() => { if (active) void refreshContext().catch(() => { if (active) setContextError('Linked evidence is unavailable on this device. Existing references are retained.'); }); });
