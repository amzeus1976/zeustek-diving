...
const connectionSecretReview = (value: unknown) => {
  try { return /gmail-connection-secret|encryptedRefreshToken|refresh_token|client_secret/i.test(JSON.stringify(value)); }
  catch { return true; }
};
export async function localBackupPayload() {
  ...
  const accountModule = `dive:${account}`;
  const snapshot = await zeustekDb.transaction('r', [zeustekDb.entities, zeustekDb.events, zeustekDb.attachments, zeustekDb.diveImages, zeustekDb.settings, zeustekDb.entityHeads], async () => {
    const entities = (await zeustekDb.entities.where('module').equals(accountModule).toArray())
      .filter(row => (DIVE_RECORD_KINDS as readonly string[]).includes(row.entityType));
    const accepted = new Set(entities.map(row => row.entityId));
    return { entities,
      events: await zeustekDb.events.filter(event => event.module === accountModule && accepted.has(event.entityId)).toArray(),
      heads: await zeustekDb.entityHeads.where('entityId').anyOf(entities.map(row => row.entityId)).toArray(),
      attachments: await zeustekDb.attachments.filter(row => accepted.has(row.entityId) || row.entityId.startsWith(`${accountModule}:computer-import:`) || row.entityId.startsWith(`${accountModule}:computer-profile:`)).toArray(),
      images: await zeustekDb.diveImages.where('account').equals(account).toArray(),
      pending: await zeustekDb.settings.where('key').startsWith(`pending:${accountModule}:`).filter(row=>accepted.has(row.key.slice('pending:'.length))).toArray(),
      reviews: await zeustekDb.settings.filter(row=>(row.key.startsWith(`conflict-archive:${accountModule}:`)||row.key.startsWith(`backup-review:${accountModule}:`))&&!connectionSecretReview(row)).toArray() };
  });
  return { format: 'zeustek-dive-local-data', version: 2, account, exportedAt: new Date().toISOString(),
    coverage: 'Records, history, pending edits, card/profile images and locally staged/imported computer evidence available on this device. Gallery media stored only in the cloud and server connection credentials are not included. Reconnect the Dive News mailbox after a restore.',
...
export async function restoreLocalPayload(value: unknown) {
  ...
  const account = currentDiveAccount(); const accountModule = `dive:${account}`;
  ...
  for (const event of data.events) {
    if (!event || event.module !== accountModule || !ids.has(event.entityId) || typeof event.eventId !== 'string' || eventIds.has(event.eventId) || !Array.isArray(event.parents) || !event.parents.every(id => typeof id === 'string') || !Number.isSafeInteger(event.lamport) || event.lamport < 1 || await recordHash(event.record) !== event.recordHash) throw invalid();
    eventIds.add(event.eventId);
  }
  for (const head of data.heads) if (!ids.has(head.entityId) || !eventIds.has(head.eventId)) throw invalid();
  for (const pending of data.pending) if (typeof pending.key !== 'string' || !pending.key.startsWith(`pending:${accountModule}:`) || !ids.has(pending.key.slice(8))) throw invalid();
  if(data.reviews!==undefined&&(!Array.isArray(data.reviews)||data.reviews.some(row=>!row||typeof row.key!=='string'||!(row.key.startsWith(`conflict-archive:${accountModule}:`)||row.key.startsWith(`backup-review:${accountModule}:`))||connectionSecretReview(row))))throw invalid();
  ...
  await zeustekDb.transaction('rw', [zeustekDb.entities, zeustekDb.events, zeustekDb.entityHeads, zeustekDb.eventParents, zeustekDb.attachments, zeustekDb.diveImages, zeustekDb.settings, zeustekDb.syncState], async () => {
    for(const review of data.reviews??[]) {
      ...
      else if(JSON.stringify(existing.value)!==JSON.stringify(review.value))await zeustekDb.settings.put({key:`backup-review:${accountModule}:archive:${crypto.randomUUID()}`,value:review.value});
    }
    ...
    for (const [index, row] of data.entities.entries()) {
      ...
      await zeustekDb.settings.put({key:`cached:${accountModule}:${row.entityType}`,value:true});
      const queued = data.pending.find(item => item.key === `pending:${row.entityId}`);
      const record = row.record as Record<string,JsonValue> | null;
      const pending = queued ?? {key:`pending:${row.entityId}`,value:{id:row.entityId.slice(accountModule.length+1),kind:row.entityType,record:row.record,baseModifiedAt:record?.modifiedAt ?? null,token:crypto.randomUUID(),state:'pending'}};
