...
export async function checkDiveCloud() {
  const entries = await Promise.all(['dive','equipment','cylinder-fill','gas-analysis','site','trip','dive-trip','certification','currency-policy','reference-requirement-set'].map(async kind => {
