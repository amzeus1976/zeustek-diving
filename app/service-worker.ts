...
registerRoute(new NavigationRoute(new NetworkFirst({
  cacheName:'zeustek-navigation-v13',networkTimeoutSeconds:1,
  plugins:[{
...
registerRoute(({ request,url }) => url.origin===self.location.origin&&(request.destination === 'script' || request.destination === 'style'), new CacheFirst({ cacheName: 'zeustek-static-v12' }));
