...
registerRoute(({request,url})=>request.mode==='navigate'&&/^\/(?:signin-with-chatgpt|signout-with-chatgpt|callback)(?:\/|$)/.test(url.pathname),async({request})=>{
  await Promise.all(['zeustek-navigation-v1','zeustek-navigation-v2'].map(name=>caches.delete(name)));
  return fetch(request);
});
registerRoute(new NavigationRoute(new NetworkFirst({
  cacheName:'zeustek-navigation-v2',networkTimeoutSeconds:1,
  plugins:[{
...
registerRoute(({ request,url }) => url.origin===self.location.origin&&(request.destination === 'script' || request.destination === 'style'), new CacheFirst({ cacheName: 'zeustek-static-v1' }));
