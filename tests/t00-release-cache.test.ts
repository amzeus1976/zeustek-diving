...
it('advances the Stage 9 release and isolates its navigation and static caches', () => {
  expect(version).toBe('1.0.59');
  expect(appChangelog[0]?.version).toBe(version);
  const worker = readFileSync(new URL('../app/service-worker.ts', import.meta.url), 'utf8');
  expect(worker).toContain("cacheName:'zeustek-navigation-v9'");
  expect(worker).toContain("cacheName: 'zeustek-static-v8'");
  expect(worker).toContain("'zeustek-navigation-v9'].map(name=>caches.delete(name))");
});
