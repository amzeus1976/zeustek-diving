...
it('advances the Stage 9 release and isolates its navigation and static caches', () => {
  ...
  expect(worker).toContain("cacheName:'zeustek-navigation-v11'");
  expect(worker).toContain("cacheName: 'zeustek-static-v10'");
  expect(worker).toContain("'zeustek-navigation-v11'].map(name=>caches.delete(name))");
});
