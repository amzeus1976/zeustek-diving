...
describe('SNAG-EQUIP-001 Overview Kit Status projection', () => {
  it('shows only active scheduled-service items with a real due date', () => {
    const items = [
      ...
      equipment('no-date', { nextServiceAt: '', purchasedAt: '', lastServiceAt: '', serviceIntervalMonths: null }),
    ];
  ...
  it('orders overdue first and then future dates soonest-first', () => {
    ...
    expect(overviewServiceItems([later, overdue, sooner], []).map((item) => item.entityId))
      .toEqual(['overdue', 'sooner', 'later']);
  });

  it('does not infer serviceability from an equipment category', () => {
    const wetsuit = equipment('wetsuit', { category: 'Wetsuit', serviceRequired: false, nextServiceAt: '2028-01-01' });
...
describe('SNAG-EQUIP-003 canonical equipment event history', () => {
  ...
  it('resolves an issue without changing its canonical entity id', async () => {
    ...
    const before = (await listEquipmentEvents()).find((event) => event.entityId === saved.id)!;
    await resolveEquipmentEvent(before, '2026-09-14', 'Hose replaced and leak test passed.');
    const after = (await listEquipmentEvents()).find((event) => event.entityId === saved.id)!;
    ...
    expect(await zeustekDb.events.where('entityId').equals(`dive:equipment-owner-snags:${saved.id}`).count()).toBe(2);
  });
  ...
  it('only changes scheduled-service baseline when explicitly requested', async () => {
    ...
    expect((await listEquipment())[0]).toMatchObject({ lastServiceAt: '2025-08-11', nextServiceAt: '2027-08-11' });

    await updateEquipmentServiceBaseline('reg-a', '2026-09-14');
    expect((await listEquipment())[0]).toMatchObject({ lastServiceAt: '2026-09-14', nextServiceAt: '2028-09-14' });
  });

  it('retains equipment events through local backup and restore', async () => {
    await saveEquipmentEvent({ equipmentId: 'reg-a', eventType: 'inspection', title: 'Bench inspection', occurredAt: '2026-09-14', status: 'resolved' });
...
describe('Equipment UI precode integration', () => {
  it('prepares the Overview projection, maintenance log and scoped editor layout', () => {
    const script = readFileSync('scripts/apply-equipment-owner-snags-precode.mjs', 'utf8');
    expect(script).toContain('overviewServiceItems(equipment, dives, 8)');
    expect(script).toContain('No equipment currently requires scheduled servicing.');
    expect(script).toContain('<EquipmentMaintenanceLog equipment={viewing} onEquipmentChanged={refresh} />');
  ...
  it('keeps the detail preview bounded to five events and exposes the full log', () => {
    const component = readFileSync('components/equipment-maintenance-log.tsx', 'utf8');
    expect(component).toContain('latestEquipmentEvents(events, equipment.entityId, 5)');
    expect(component).toContain('More… view full log');
    expect(component).toContain('Resolve issue');
    expect(component).toContain('Update scheduled-service baseline from this service');
  });
});
