...
- Substantive record editors still using `AccessibleDialog editable`: import review, calendar bookings, skill evidence, and media/photo operations. Apply Loadout and Cylinder detail are multi-action operational dialogs with existing inline saves; classify their actions individually before migration. The Dive detail viewer has interactive evidence review and must be inspected before classifying it; do not remove its navigation protection mechanically.
