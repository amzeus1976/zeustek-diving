...
- Substantive record editors still using `AccessibleDialog editable`: conservation activity/programme, Skill catalogue/bulk/CSV, cylinder/loadout, import review, calendar bookings, skill evidence, technical reference, and media/photo operations. Convert in bounded groups only after checking each save/validation flow. The Dive detail viewer has interactive evidence review and must be inspected before classifying it; do not remove its navigation protection mechanically.
