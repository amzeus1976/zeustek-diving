...
## 2026-09-24 People / Dive Entity correction — local checkpoint

ENTITY-01 through ENTITY-06 are locally implemented and marked LOCAL PASS. The existing Person/Operator stores remain canonical; relationship kinds are additive. Full regression 129 files/742 tests PASS, typecheck/build/PWA/focused lint PASS, four browser widths with zero overflow/application console errors, six-credential client scan zero matches, nine protected hashes unchanged. Sites126 and GitHub main remain on the previously accepted source; this correction is not published. See `people-dive-entity-implementation-evidence.md` and `HANDOVER.md` for release prerequisites.
...
## Stage 8 — locally complete; scoped release candidate under final review

- Redacted ZeusTek application diagnostics use fixed codes and numeric-only details. The Site Logs viewer has domain, severity and period filters, bounded retention, CSV/JSON downloads and Clear. It labels these as local application diagnostics rather than hosting security logs. Raw errors and legacy log details cannot enter the safe export model.
- Site Configuration now has independent cards for equipment/training lists, category icons, training agency logos, Insights awards, My Maps, Dive numbering, Weather & Conditions and News/inbox. Domain saves read latest settings, validate only their own fields, preserve unrelated/unknown values, and survived isolated fixture save/reload. Provider status remains truthful: Met Office access denied, Copernicus binding absent and SwellCloud approval/access conditional.
- Diver Summary PDF/DOCX/TXT/CSV/JSON share one owner-selected privacy projection. Equipment is included when selected; certification numbers and images require separate opt-in. The generated PDF/DOCX bytes and content, CSV formula safety, image-failure fallback and all five browser download controls passed local checks.
- Cloud backup queries only canonical record kinds, so the encrypted Gmail connection row is never read or serialised. Legacy cloud restore skips that row while restoring ordinary records. Local backup filters unknown/server-only entities, events, pending records and secret-bearing review metadata; legacy secret-bearing reviews are rejected on restore. Backups tell the owner to reconnect Gmail after restoration. Dummy credentials only were used in tests.
- A cloud record GET no longer writes household membership/schema during read-only smoke. Authenticated read and new-account empty-state tests pass. POST/DELETE registration and write behaviour stay intact.
- New Stage8 focused tests: 24; complete regression 121 files / 697 tests PASS; typecheck PASS; build/PWA PASS; new/security-path targeted lint PASS. The modified dashboard/offline-platform monolith has 80 retained lint diagnostics versus 100 at the pre-Stage8 checkpoint; remaining monolith lint debt belongs to the Stage9 backlog. Version 1.0.53 and changelog agree. 748 complete icons retained; 17 selectively precached. Six local server credential values: zero client/staged-diff matches.
- Browser verification on Admin, Configuration, Export, Insights, Dive Centres, Dive Planning, Gas Planning and News at 390/820/1024/1440: all relevant headings rendered, zero page-level overflow and zero application console errors. Local production-built HTTP smoke: shell/API/backup/service worker/manifest all 200 with isolated fixture identity; one isolated fixture Dive created through the built API, backup contained it and no connection-secret row. No production owner record was touched.
- Nine protected calculation files remain byte-identical to the approved baseline. Local 75 Dives plus one Gas Plan content fingerprint unchanged `221e9ff72ab5deb482b5ef0029e556c8ee7a3e667e54ff52e0671ca646411a10`; existing Plan fingerprint unchanged `edeffbe5e65a30ac2eaa54f36562ee50e754ea53f90c9ef00d468cff2f39bebe`.
- The owner authorised a scoped release through Stage8. Stage9 non-blocking editor migration/finishing, optional topic card and disabled public/API profile remain deferred in the issue matrix. At this local checkpoint, live Gmail acceptance had not run and production was still app1.0.52 / Sites124. The later release attempt and rollback are recorded below.

## Scoped release attempt — BLOCKED_DEPLOYMENT; Sites124 restored

- Candidate source `18aa824` passed the final 121-file/697-test regression, typecheck, build/PWA, targeted lint, privacy scan, nine protected hashes, local production-built smoke and four-width browser gate. The Site source repository received that exact source; an archive-backed Sites125 version was saved and deployed once.
- Live app1.0.53 smoke confirmed Admin/configuration, export actions, completed Stages 0–7 and 5W screens, four viewport widths, zero application console errors and no page overflow. New backup excluded the legacy encrypted Gmail connection row; all 43 canonical owner kinds matched pre-release counts and content fingerprints, including 66 Dives and one Gas Plan.
- The one authorised Gmail acceptance sync was invoked once. Durable run `fdeda6a4-5504-49d0-881d-d1795839955b` failed with safe `upstream_failure` diagnostic, zero imported/updated stories and no reconnect requirement. No repeat was attempted. This mandatory acceptance failure blocks publication acceptance.
- Sites124 was restored through deployment `appgdep_6ab455d58c248191840e6ed604373586`; live app1.0.52 and all 43 canonical owner-kind fingerprints were verified afterward. The old backup endpoint again includes its encrypted connection row. GitHub main remains `b2e86827c19239776c80b51fc93067c7dc4b18ac`, with no T14 PR or main sync. Full IDs, hashes, evidence and next steps are in `scoped-release-attempt.md`. Do not repeat live Gmail sync without new explicit owner authorisation.

## Owner-approved Gmail release exception and new candidate

- On 2026-09-24 the owner authorised publication without a successful Gmail sync acceptance. The recorded run remains `FAILED — upstream_failure` and NEWS-02 is `PARTIAL`, explicitly excepted for this scoped release only. No second live Gmail operation is authorised.
- The new app1.0.54 candidate keeps the prior Stage8 source, adds a bounded Gmail sync lock at the authenticated API and News UI, reports disabled status, retains encrypted connection/cached stories/public feeds and advances navigation/static cache identifiers. There is no automatic or scheduled Gmail path.
- Isolation tests were written first and failed against the old code as expected. After implementation, 11 focused Gmail tests passed; the full 122-file/698-test regression, typecheck, production build/PWA, targeted lock lint, version/cache gate, six-credential client-bundle privacy scan, all nine protected hashes and local owner-fixture fingerprints passed. The changed News screen rendered the disabled notice at 390/820/1024/1440 with no page-level overflow or application console errors. The preceding Stage8 candidate's four-width checks cover its otherwise unchanged screens. The current live Sites124 recovery deployment is healthy. A fresh pre-deployment backup has 43 canonical kinds/6,362 records: 66 Dives and one Gas Plan, with canonical aggregate SHA-256 `062d04a608dd1f531e7a9de3f9b2a82502797c0eba2b76105ac37eacdb05751a`. Finish exact-source packaging, deployment and production acceptance; do not invoke the Gmail sync endpoint.
