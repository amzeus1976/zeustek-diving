...
Choose the authoritative 24-item list before implementation: **PADI-published 2026 list with current-instructor verification** (recommended), or the issue's differing list as a **custom practice circuit with no PADI pass claim**. If the owner has a newer instructor-issued list, review its edition and exact differences first. This is an explicit planning/source gate; no circuit code or owner data changes occur until the decision.
